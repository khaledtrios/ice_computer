        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#CreateTicket">
                        <i class="fa-solid fa-plus"></i> Créer un Ticket
                    </button>
                </div>
                <div class="card-body">
                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="mb-0" id="CreateTicketLabel">Créer un Ticket</h5>
                        ...
                        <div class="modal-body custom-input">
                            <form id="createTicketForm" class="row g-3" enctype="multipart/form-data">
                                @csrf
                                <div class="col-md-12">
                                    <label class="form-label" for="ticketObjet">Objet</label>
                                    <input class="form-control" id="ticketObjet" name="objet" type="text" placeholder="Entrez l'objet du ticket" required>
                                </div>

                                <div class="col-md-12">
                                    <label class="form-label" for="ticketMessage">Message</label>
                                    <textarea class="form-control" id="ticketMessage" name="message" rows="4" placeholder="Entrez le message" required></textarea>
                                </div>
                            </form>
                        </div>
                        ...
                        <div class="modal-body custom-input">
                            <form id="editTicketForm" class="row g-3" enctype="multipart/form-data">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input class="form-control" id="edit_ticketObjet" name="objet" type="text" placeholder="Entrez l'objet du ticket" required>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-12">
                                    <label class="form-label" for="edit_ticketMessage">Message</label>
                                    <textarea class="form-control" id="edit_ticketMessage" name="message" rows="4" placeholder="Entrez le message" required></textarea>
                                </div>
                            </form>
                        </div>
                        <div class="modal-footer">
        ...
        $(document).ready(function () {
            ...
            if (dt_ticket_table.length) {
                var dt_tickets = dt_ticket_table.DataTable({
                    ...
                    serverSide: true,
                    ajax: {
                        url: "{{ route('boutique.apigettickets') }}",
                        type: "GET",
                        error: function (xhr) {
                    ...
                    columnDefs: [
                        ...
                        },
                        {
                            targets: 3,
                            render: function (data, type, full) {
                                var badgeClass = full.status === 'Ouvert' || full.status === 'En cours' ? 'badge-success' : 'badge-danger';
                        ...
                        },
                        {
                            targets: 4,
                            render: function (data, type, full) {
                                var badgeClass = full.is_open === 'Oui' ? 'badge-success' : 'badge-danger';
                                var statusText = full.is_open === 'Oui' ? 'Ouvert' : 'Fermé';
                        ...
                        },
                        {
                            targets: 6,
                            render: function (data, type, full) {
                                var linkShow = '{{ route('boutique.tikets.show', ':id') }}'.replace(':id', encodeURIComponent(full.id));
                                return (
                                    '<ul class="action d-flex align-items-center list-unstyled gap-2 m-0">' +
                                    '<li><a href="' + linkShow + '" class="btn btn-sm btn-icon btn-info" title="Voir le ticket"><i class="fa-solid fa-eye"></i></a></li>' +
            ...
            $('#saveTicket').on('click', function () {
                ...
                if (form[0].checkValidity()) {
                    ...
                    $.ajax({
                        url: "{{ route('boutique.tickets.store') }}",
                        type: "POST",
                        dataType: "json",
                        data: formData,
                        processData: false,
            ...
            $('#updateTicket').on('click', function () {
                ...
                if (form[0].checkValidity()) {
                    ...
                    $.ajax({
                        url: "{{ route('boutique.tickets.update', ':id') }}".replace(':id', id),
                        type: "POST",
                        dataType: "json",
                        data: formData,
                        processData: false,
            ...
            $('#ticketsTable').on('click', '.delete-record', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('boutique.tickets.destroy', ':id') }}".replace(':id', id),
                            type: "DELETE",
                            dataType: "json",
